...
`dotnet build -c Release` in `src/<Mod>`. No game DLLs are in this repo: `src/Directory.Build.props` reads R.E.P.O.'s managed assemblies from `refs/Managed` next to `src/` (git-ignored, copy them from your game) or, if that doesn't exist, from `C:\Program Files (x86)\Steam\steamapps\common\REPO\REPO_Data\Managed`. Override with `-p:ManagedPath=...`. BepInEx comes from NuGet. Package with `tcli build --config-path src/<Mod>/thunderstore.toml` after copying the built DLL next to it. See NOTES.md for details.
